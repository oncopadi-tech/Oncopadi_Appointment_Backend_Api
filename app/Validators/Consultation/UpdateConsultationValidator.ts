...
const schemaToCreate = {
  ...
  referral: schema.object.optional().members({
    comment: schema.string.optional(),
    hospitalId: schema.number.optional([
      rules.exists({ table: 'hospitals', column: 'id' })
    ]),
    patientId: schema.number([rules.exists({ table: 'users', column: 'id' })])
  })
}
