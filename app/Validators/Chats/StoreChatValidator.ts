...
const validations = {
  message: schema.string.optional({}, [rules.requiredIfNotExists('file')]),
  to: schema.number.optional([
    rules.exists({ table: 'users', column: 'id' }),
    rules.requiredIfNotExists('chatGroupId')
  ]),
  chatGroupId: schema.number.optional([
    rules.exists({ table: 'chat_groups', column: 'id' }),
    rules.requiredIfNotExists('to')
  ]),
  file: schema.file.optional({ size: '2mb' }, [
    rules.requiredIfNotExists('message')
  ])
}

export default class StoreChatValidator {
